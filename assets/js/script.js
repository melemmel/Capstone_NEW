        ...
        ? Object.entries(entries)
            .filter(([_, entry]) => {
              try {
                ...
                const entryMonth = date.toLocaleString("default", {
                  month: "long",
                });
                  ...
                  <tbody>
                    ${
                      ...
                      <tr>
                        ...
                        <td>
                          <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editEntryModal" onclick="editEntry('${
                            entry.key
                          }')">
    ...
    .then(() => {
      ...
      setTimeout(() => {
        const accordionButton = document.querySelector(
          `button[data-bs-target="#${currentMonth}"]`
        );
        if (accordionButton) {
...
document.getElementById("updateEntry").addEventListener("click", function () {
  ...
  entriesRef
    ...
    .then(() => {
      ...
      const monthKey =
        entryDate.toLocaleString("default", { month: "long" }).toLowerCase() +
...
function loadArchivedEntries() {
  ...
  entriesRef
    .once("value", (snapshot) => {
      const entries = snapshot.val();
      const archivedYearsContainer = document.getElementById(
        "archivedYearsContainer"
      );
                    ...
                    <tbody>
                      ${yearEntries
                        ...
                        <tr>
                          ...
                          <td>
                            <button class="btn btn-sm btn-info" onclick="viewArchivedEntry('${
                              entry.key
                            }')">
                              <i class="bi bi-eye"></i>
                            </button>
                            <button class="btn btn-sm btn-warning" onclick="restoreEntry('${
                              entry.key
                            }')">
...
document.getElementById("searchForm").addEventListener("submit", function (e) {
  ...
  const controlNumber = document
    .getElementById("searchControlNumber")
    .value.trim();
      ...
      if (entries) {
        ...
        results.forEach((entry) => {
          ...
          row.innerHTML = `
            ...
            <td>${entry.subject}</td>
            <td>${entry.endorsedTo}</td>
            <td><span class="badge bg-${getStatusColor(entry.status)}">${
            entry.status
          }</span></td>
            <td>
              <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editEntryModal" onclick="editEntry('${
              ...
              <button class="btn btn-sm btn-info" onclick="viewEntryDetails('${
                entry.key
              }')">
                <i class="bi bi-eye"></i>
              </button>
...
document.getElementById("searchInput")?.addEventListener("input", function (e) {
  ...
  const originalText =
    popupText.getAttribute("data-original-text") || popupText.textContent;
...
function searchDocument() {
  const controlNumber = document
    .getElementById("controlNumberInput")
    .value.trim();
    ...
    .then((snapshot) => {
      if (!found && snapshot && snapshot.exists()) {
        ...
        document.getElementById("popup-text").textContent =
          "No record found for this control number.";
      }
    })
    .catch((error) => {
...
function displayResults(data) {
  const searchResults = document.getElementById("search-results");
  document.getElementById("result-ctrl-no").textContent =
    data.controlNumber || data.ctrl_No || "N/A";
  document.getElementById("result-endorsed").textContent =
    data.endorsedTo || data.endorsed || "N/A";
